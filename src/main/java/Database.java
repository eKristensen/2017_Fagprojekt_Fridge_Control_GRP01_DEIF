	...
	public static void CommitCache() throws java.lang.ClassNotFoundException {
		try {
			...
			for (int i = 0; i < sendcache.size(); i++) {
				...
				if (measurements.containsKey("current_average")) sql = sql + ", '" + measurements.get("current_average") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("current_neutral")) sql = sql + ", '" + measurements.get("current_neutral") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("current_phase_a")) sql = sql + ", '" + measurements.get("current_phase_a") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("current_phase_b")) sql = sql + ", '" + measurements.get("current_phase_b") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("current_phase_c")) sql = sql + ", '" + measurements.get("current_phase_c") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("frequency")) sql = sql + ", '" + measurements.get("frequency") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("reactive_power_a")) sql = sql + ", '" + measurements.get("reactive_power_a") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("reactive_power_b")) sql = sql + ", '" + measurements.get("reactive_power_b") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("reactive_power_c")) sql = sql + ", '" + measurements.get("reactive_power_c") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("voltage_a_n")) sql = sql + ", '" + measurements.get("voltage_a_n") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("voltage_b_n")) sql = sql + ", '" + measurements.get("voltage_b_n") + "'"; else sql = sql + ", NULL";
				if (measurements.containsKey("voltage_c_n")) sql = sql + ", '" + measurements.get("voltage_c_n") + "'"; else sql = sql + ", NULL";
				sql = sql + ")";
			}
			sendcache.clear();
			sql = sql + ";";
